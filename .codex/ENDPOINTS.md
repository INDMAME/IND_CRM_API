...
- POST /api/ia/service/help/ask (Authorize)
  ...
  `selectedModuleId` debe ser un ID devuelto por `GET /api/help/catalog` y actua como scope estricto. Sin `selectedTopicId`, el API entrega a una unica llamada de OpenAI todos los temas y chunks publicados de ese modulo para que interprete la intencion y seleccione la evidencia relevante; el ranking lexico queda solo como diagnostico y ya no descarta ni preselecciona la pregunta, incluso cuando un seguimiento no contiene tokens independientes. Este modo elimina primero el historial antiguo si necesita liberar presupuesto y falla de forma segura si el modulo completo no cabe, sin responder desde evidencia parcial. Un modulo inexistente, un modulo sin temas o un `selectedTopicId` que no pertenece al modulo seleccionado devuelve `notDocumented` sin llamar a OpenAI.

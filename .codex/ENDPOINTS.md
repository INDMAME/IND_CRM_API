...
- PUT /api/crm/expensesheets/{hojaGastosId} (Authorize + X-IND-Company + X-IND-AxUserId)
  ...
  Notificaciones best-effort: si `INDCRM_EXPENSE_NOTIFICATIONS_ENABLED=true`, el API lee snapshot antes/despues solo cuando llega `expenseSheetStatus`; transiciones configuradas `ExpenseSheetApprovalRequested` y `ExpenseSheetApproved` llaman a `IND_INTERNAL_API` `POST /api/internal/v1/mail/messages`. `X-IND-AxUserId` se pasa a Axapta como `ActorAxUserId`; si actor CRM y propietario de la hoja coinciden, se salta el email como autogestionado. Fallos de email se registran y no cambian la respuesta del PUT.

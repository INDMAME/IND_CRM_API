...
- Resolve `IND_INTERNAL_API` connection values from the same source used by the working manual jobs:
  - `INDDefaultParameters.InternalAPIUrlService`
  - `INDDefaultParameters.APIUserId`
  - `INDDefaultParameters.INDAPIIntServiceUser().Password`
- Use `ParameterTable.INDInternalApiBaseUrl`, `INDInternalApiClientId`, and `INDInternalApiClientSecret` only as temporary fallback values if the `INDDefaultParameters` values are empty.
...
```text
ExpenseSheetApprovalRequested  From: CRMHojaGastosTable.UserId owner, To: direct manager from CRMUsuarioSubordinadoTable.UserIdJefe
...
For `ExpenseSheetApprovalRequested`, resolve the manager with `CRMUsuarioSubordinadoTable` where `UserIdSubordinado == CRMHojaGastosTable.UserId`. If no manager is found, skip and log; do not send to the actor as fallback unless business explicitly changes the rule.
...
6. Resolve recipient language from `SysUserInfo.Language` through the CRM user -> AX user relation.
7. Resolve active `INDEmailTemplates` by `TargetModule + LanguageId + today()`.
8. Render `SubjectTemplate` and `HtmlTemplate` with the fixed `strFmt` placeholder order.
9. If no active template exists, send a plain-text fallback with the link.
10. Call `INDCRMUtilityService::sendInternalApiMail`.
11. Catch/log send failures and return `false`; never block the status change.
...
resolveAxUserIdFromCrmUserId(crmUserId)
resolveLanguageIdFromCrmUserId(crmUserId)
resolveExpenseSheetManagerCrmUserId(ownerCrmUserId)
resolveExpenseSheetTemplateTargetModule(eventType, toStatus)
buildEmailLogoSrc(logoBase64)
buildExpenseSheetAmountText(expenseSheet)
buildExpenseSheetEventDisplayText(eventType)
buildExpenseSheetMonthAbbrev(date)
renderExpenseSheetTemplate(template, expenseSheet, eventType, link, logoSrc, isHtml)
buildExpenseSheetFallbackTextBody(message, expenseSheet, eventType, link)
...
## INDEmailTemplates requirements

`INDEmailTemplates` is the source for subject and HTML body.

Required behavior:

- Keep fields `FromDate` and `ToDate`; remove any dependency on `IsActive`.
- Add/keep a non-unique index over `TargetModule, LanguageId, FromDate, ToDate`.
- Add `findValid(TargetModule, LanguageId, asOfDate)` to return the current valid template.
- Add `existsOverlap(TargetModule, LanguageId, FromDate, ToDate, excludeRecId)` and call it from `validateWrite` to prevent overlapping validity ranges for the same target module and language.
- Treat empty `ToDate` as open-ended.
- Keep `Logo` as raw Base64 only; do not store the Base64 inside `HtmlTemplate`.

Target module mapping is based on destination status:
...
Do not add `CRMRejectionCancelled` to `INDEmailTemplateTargetModule`; `Rejected -> InReview` uses `CRMInReview` and can show a different visible event through the `%2` parameter.

Template placeholders must use this fixed `strFmt` order:
...
%5  Description
%6  CRM detail link
%7  Button text: Abrir hoja de gastos
%8  Year
%9  Spanish abbreviated month, uppercase
%10 Day, two digits
%11 Status comments
%12 Logo src, formatted as data:image/png;base64,... from INDEmailTemplates.Logo
```
...
.codex/ENDPOINTS.md
docs/plans/2026-05-22-expense-sheet-email-deeplinks-design_API.md
.codex/AX_INDCRMExpenseSheetService_CHANGES_2026-05-28.md
...
- `Draft -> InReview` calls AX notification logic once and sends to the direct manager from `CRMUsuarioSubordinadoTable`.
...
- Missing email/link/template logs and does not block.
- Active template is resolved by `TargetModule + SysUserInfo.Language + FromDate/ToDate`.
- If no template exists, plain-text fallback still sends the link.
- HTML templates use `%12` for logo `src` and do not contain embedded Base64.

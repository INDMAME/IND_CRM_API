...
{
    /// <summary>
    /// Secondary exchange-rate provider backed by Frankfurter latest endpoint.
    /// </summary>
    public class FrankfurterExchangeRateProvider : IRawExchangeRateProvider
    {
        private const string Endpoint = "https://api.frankfurter.app/latest";
        private static readonly Regex IsoCurrencyRegex = new Regex("^[A-Z]{3}$", RegexOptions.Compiled);
        private static readonly HttpClient SharedHttpClient = CreateSharedHttpClient();

        private readonly IAxLogger _logger;

        public FrankfurterExchangeRateProvider(IAxLogger logger)
        {
        ...
        public ExchangeRateResult GetRate(string baseCurrency, string targetCurrency, DateTime date)
        {
            ...
            var requestUrl = BuildRequestUrl(normalizedBase, normalizedTarget);
            ...
            try
            {
                ...
                using (var response = SharedHttpClient.SendAsync(request, HttpCompletionOption.ResponseContentRead).GetAwaiter().GetResult())
                {
                    ...
                    {
                        _logger.Log($"[EXCHANGE-FRANKFURTER] GET {requestUrl} -> {(int)response.StatusCode}", AxaptaSessionManager.LogLevel.Warning);
                    ...
                    {
                        _logger.Log($"[EXCHANGE-FRANKFURTER] Parse error reason={parseError}", AxaptaSessionManager.LogLevel.Warning);
            ...
            {
                _logger.Log($"[EXCHANGE-FRANKFURTER] Timeout {ex.Message}", AxaptaSessionManager.LogLevel.Warning);
                return BuildFailure(ExchangeRateProviderErrorCodes.ProviderError, date.Date);
            }
            catch (HttpRequestException ex)
            {
                _logger.Log($"[EXCHANGE-FRANKFURTER] Request error {ex.Message}", AxaptaSessionManager.LogLevel.Warning);
                return BuildFailure(ExchangeRateProviderErrorCodes.ProviderError, date.Date);
            }
            catch (Exception ex)
            {
                _logger.Log($"[EXCHANGE-FRANKFURTER] Unexpected error {ex.Message}", AxaptaSessionManager.LogLevel.Warning);
                return BuildFailure(ExchangeRateProviderErrorCodes.ProviderError, date.Date);
            }
        }

        private static string BuildRequestUrl(string baseCurrency, string targetCurrency)
        {
            return $"{Endpoint}?from={Uri.EscapeDataString(baseCurrency)}&to={Uri.EscapeDataString(targetCurrency)}";
        }
        ...
        private static HttpClient CreateSharedHttpClient()
        {
            ...
            var timeoutSeconds = ReadTimeoutSeconds("ExchangeRate:FrankfurterTimeoutSeconds", 5);

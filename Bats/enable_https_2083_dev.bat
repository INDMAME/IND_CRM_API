...
title Habilitar HTTPS IND_CRM_API 2083 DEV
cd /d "%~dp0"

REM ------------------------------------------------------
REM Configure HTTP.sys for DEV HTTPS on port 2083.
REM
REM Guardrails:
REM - Run this script as Administrator.
REM - The machine must already be configured as DEV.
REM - The PFX stays outside the repository by default.
...
set "PFX_PATH=%INDCRM_DEV_PFX_PATH%"
if not "%PFX_PATH%"=="" set "PFX_PATH_SOURCE=%PFX_PATH_ENV_VAR%"
if "%PFX_PATH%"=="" set "PFX_PATH=C:\INDAxaptaConfigAPI\dev.insertec.biz\dominio.pfx"
set "PFX_PASSWORD=%INDCRM_DEV_PFX_PASSWORD%"
set "BACKUP_FILE=%TEMP%\IND_CRM_API_https_%TARGET_ENV%_%PORT%_backup.txt"
set "LEGACY_BACKUP_FILE=%TEMP%\IND_CRM_API_https_%TARGET_ENV%_%LEGACY_PORT%_backup.txt"

REM Optional overrides:
REM   enable_https_2083_dev.bat "C:\custom\path\dominio.pfx"
REM   enable_https_2083_dev.bat "C:\custom\path\dominio.pfx" "password"
if not "%~1"=="" (
    set "PFX_PATH=%~1"
...
if not "%LEGACY_PORT%"=="%PORT%" (
    netsh http show sslcert ipport=0.0.0.0:%LEGACY_PORT% > "%LEGACY_BACKUP_FILE%" 2>&1
    echo Legacy sslcert state saved to: %LEGACY_BACKUP_FILE%
)
echo.
...
if not "%LEGACY_PORT%"=="%PORT%" (
    netsh http delete sslcert ipport=0.0.0.0:%LEGACY_PORT% >nul 2>&1
    netsh http delete urlacl url=https://%HOST%:%LEGACY_PORT%/ >nul 2>&1
)

netsh http add urlacl url=https://%HOST%:%PORT%/ user="%SERVICE_USER%"
if errorlevel 1 (
    echo ERROR: Failed to create the DEV URL ACL for https://%HOST%:%PORT%/
    goto :fail
)

netsh http add sslcert ipport=0.0.0.0:%PORT% certhash=%CERT_THUMBPRINT% appid="%APP_ID%" certstorename=MY
if errorlevel 1 (
    echo ERROR: Failed to bind the DEV certificate to 0.0.0.0:%PORT%
    goto :fail
)

echo.
echo HTTPS DEV configured successfully.
echo Verify with:
...
if not exist "%PFX_PATH%" (
    ...
    echo The DEV operational certificate is normally managed outside the repository.
...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command ^
  ...
  "if ([string]::IsNullOrWhiteSpace($plainPassword)) { $securePassword = Read-Host 'Enter DEV PFX password' -AsSecureString } else { $securePassword = ConvertTo-SecureString $plainPassword -AsPlainText -Force };" ^
...
if not defined CERT_THUMBPRINT (
    echo ERROR: Failed to import the DEV certificate or resolve a valid thumbprint for %HOST%.
    exit /b 1
)
...
:fail
echo.
echo HTTPS DEV configuration did not finish.

...
if not defined CERT_THUMBPRINT (
    echo ERROR: Failed to import the DEV certificate or read its thumbprint.
    pause
    exit /b 1
)
...
REM netsh advfirewall firewall add rule name="IND_CRM_API HTTPS DEV %PORT%" dir=in action=allow protocol=TCP localport=%PORT%

...
- `Job_INDDVUsers` prints both `ctrlGetVisibleUsers` lines and the `ctrlResolveVisibleAxUserIdSet` set; users without AX user should not appear in either output after the 2026-06-09 adjustment.
...
- Add or identify one visible hierarchy person without `INDPersonaTable.UserId` and confirm it is excluded from `Job_INDDVUsers`.
